              ...
              (data, isBinary) => {
                ...
                if (isBinary) {
                  ...
                  if (
                    ...
                    // Real-time remote viewing must prefer the newest frame over
                    // building a backlog of old JPEGs. If the technician socket is
                    // still flushing a previous frame, drop this one; the agent will
                    // send another shortly. This keeps displayed video near-live.
                    if (session.tech.bufferedAmount < 256 * 1024) {
                      session.tech.send(data);
                    }
                  }
...
let frameSequence = 0;
let remoteFrameDecodeBusy = false;
    ...
    try{
      ...
      socket.onmessage = function(event){
        ...
        if(typeof event.data !== 'string'){
          ...
          // Decode only one remote frame at a time. If frames arrive faster than
          // the browser can decode them, retain only the newest pending Blob.
          // This prevents historical frames from making the technician view lag.
          latestRemoteFrame = event.data;
          if(remoteFrameDecodeBusy){
            return;
          }
          remoteFrameDecodeBusy = true;
          const frameBlob = latestRemoteFrame;
          latestRemoteFrame = null;
          const sequence = ++frameSequence;

          createImageBitmap(frameBlob).then(function(bitmap){
            ...
            remoteFrameDecodeBusy = false;
            if(latestRemoteFrame && socket && socket.readyState === WebSocket.OPEN){
              const pending = latestRemoteFrame;
              latestRemoteFrame = null;
              // Feed only the newest pending frame back through this handler.
