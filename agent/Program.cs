...
while (true)
{
    try
    {
        ...
        // A machine can log on before Wi-Fi/Ethernet has Internet access. Never let the
        // initial WebSocket connect hang indefinitely in that state: time it out and let
        // the existing outer reconnect loop try again until connectivity returns.
        using (var connectCts = new CancellationTokenSource(TimeSpan.FromSeconds(12)))
        {
...
void HandleInput(JsonElement i)
{
    ...
    // BlockInput during Maintenance is owned by the Maintenance UI thread.
    // Run technician injection on that same thread so remote control keeps working
    // while physical customer input remains hard-blocked.
    if (!MaintenanceController.DispatchTechnicianInput(Apply))
        Apply();
}
